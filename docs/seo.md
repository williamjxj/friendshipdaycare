...
- **Robots**: `src/app/robots.ts` (built-in robots.txt generation).
...
| Today’s Story     | `/community/todays-story`   | Weekly Bible stories daycare        | Yes          |
| Monthly Journal   | `/community/journal`        | Daycare monthly journal Coquitlam   | Yes          |
...
- [ ] FAQPage schema present on Enrollment (and any future FAQ sections)
...
6. Check robots at `/robots.txt` (generated from `src/app/robots.ts`).
...
- Metadata API (built-in): `src/app/layout.tsx` + per-route `metadata`
- Sitemap (built-in): `src/app/sitemap.ts`
- Robots (built-in): `src/app/robots.ts`
- Meta tags: title/description/canonical, plus Open Graph/Twitter
- Canonical example: `<link rel="canonical" href="https://example.com/products/phone" />`
